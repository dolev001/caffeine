...
import com.github.benmanes.caffeine.cache.simulator.policy.sketch.tinycache.ConcurrentKWayCacheLockFree;
...
public final class Registry {
  ...
  private static void registerSketch(Map<String, Function<Config, Set<Policy>>> factories) {
    ...
    Stream.of(ConcurrentKWayCacheLockFree.EvictionPolicy.values()).forEach(priority -> {
        String id = "concorentkway." + priority.name().toLowerCase();
        factories.put(id, config -> ConcurrentKWayCacheLockFree.policies(config, priority));
      });
